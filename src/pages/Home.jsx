...
function PortfolioOptimizer() {
  ...
  const CombinedComponent = () => {
    ...
    const riskTolerance = 10; // As specified
    ...
    useEffect(() => {
      const handleScroll = () => {
        if (isFinalPhase) return; // Lock into final phase
    ...
    // =========================
    // Mouse Movement Handling for Parallax
    // =========================
    ...
    useEffect(() => {
      const fetchArticles = async () => {
        try {
          ...
          if (!response.ok) throw new Error('Failed to fetch articles');
          const data = await response.json();
          setArticles(data.slice(0, 3)); // Get the latest 3 articles
    ...
    const prepareChartData = () => {
      if (
        ...
        !portfolioData.dashboard_data.performance
      )
        return [];
      ...
      return dates.map((date, idx) => ({
        date,
        Portfolio: portfolioSeries.values[idx] || 0,
      }));
    };
    ...
    return (
      ...
      >
        {/* 
          =============================
            MAIN HERO CONTAINER
            Occupies 300vh to allow scroll transitions
          =============================
        */}
        <div className="relative min-h-[300vh]">
          ...
          <div className="sticky top-0 h-screen flex flex-col items-center justify-center pointer-events-none">
            ...
            <div
              ...
              style={{
                ...
                }px, 0) translateY(-${transitionProgress * 50}px)`,
                opacity: phase < 3 ? 1 - transitionProgress : 0,
                transition: 'opacity 0.7s ease-out, transform 0.7s ease-out',
              }}
            >
              {/* Main Title */}
              <h1 className="text-5xl md:text-7xl font-bold mb-4">Grow your wealth</h1>
            ...
            {/* Phase 3 Content */}
            <div
              className="absolute top-0 left-0 w-full h-full flex flex-col items-center justify-center transition-opacity duration-700 pointer-events-auto"
              style={{
                opacity: phase >= 3 ? transitionProgress : 0,
              }}
            >
              {/* Static Title */}
          ...
          <div>
            ...
            <section id="portfolio-optimizer" className="py-20 bg-gray-50">
                    ...
                    <div className="w-full h-96">
                      <ResponsiveContainer width="100%" height="100%">
                        <LineChart data={chartData}>
                          <XAxis
                            dataKey="date"
                            hide={true} // Hiding X-axis as per requirements
                          />
                          <Tooltip
                            contentStyle={{
                      ...
                      {/* Percentage Label */}
                      {portfolioData.dashboard_data.performance.summary && (
                        <div className="text-center mt-4 text-2xl font-semibold text-green-700">
                          {portfolioData.dashboard_data.performance.summary}
                        </div>
                      )}
                    </div>
